...
3. **Phase 2 FULL STOP — awaiting Jeremy chat sign-off** after live verify (disk I/O + Zen fixes shipping). Do not mark COMPLETE / do not start Phase 3 until Jeremy says so in his own words.
4. **Project switcher RESOLVED (Option 3):** Home list/grid + header switcher. **Typography still OPEN.**
5. Verify live: Home triage, hierarchy, Draft chrome, true full-screen Zen (Esc), History UI, archive/typed-delete, autosave without disk I/O error (restart API after pull).
6. Protocol: [`docs/PHASE_STEP_PROTOCOL.md`](PHASE_STEP_PROTOCOL.md). In-chat verify lists; agents never invent Jeremy’s confirmation. No servers/browsers from agents.
